 ...
 */
@Component
public class ProgressGameAction extends AbstractAction {

    /**
     * Field description
     */
    private final static Logger log = LoggerFactory.getLogger(ProgressGameAction.class);
     ...
     */
    @Autowired
    public ProgressGameAction(WorkflowProcessorService workflowProcessorService) {
    ...
    public void execute(ProcessObject po) {
        if (po instanceof FsmProcessObject) {
            if (log.isDebugEnabled()) {
                log.debug("Progressing game...");
            }

            FsmProcessObject pgpo = (FsmProcessObject) po;

            workflowProcessorService.doProgressHand(pgpo.getId(), pgpo.getHandId());

            if (log.isDebugEnabled()) {
                log.debug("Progressed game successfully");
            }
        }
    }
}
