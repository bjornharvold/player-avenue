...
public class RaiseAction extends AbstractAction {
     ...
     * Field description
     */
    private final static Logger log = LoggerFactory.getLogger(RaiseAction.class);
     ...
     */
    @Autowired
    public RaiseAction(WorkflowProcessorService workflowProcessorService) {
    ...
    public void execute(ProcessObject po, Object triggerEvent) {
        if ((po instanceof FsmProcessObject) && (triggerEvent instanceof RaiseEvent)) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Gambler with id: " + event.getGamblerId() + " raises " + event.getAmount());
            }

            workflowProcessorService.doRaise(event);

            if (log.isDebugEnabled()) {
                log.debug("Gambler with id: " + event.getGamblerId() + " raised " + event.getAmount() + " successfully");
            }
        }
    }
}
