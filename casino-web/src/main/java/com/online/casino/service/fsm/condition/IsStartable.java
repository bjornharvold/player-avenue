 ...
 */
@Component
public class IsStartable extends AbstractCondition {

    /**
     * Field description
     */
    private final static Logger log = LoggerFactory.getLogger(IsStartable.class);
     ...
     */
    @Autowired
    public IsStartable(WorkflowProcessorService workflowProcessorService) {
    ...
    public boolean evaluate(ProcessObject po) {
        ...
        if (po instanceof FsmProcessObject) {
            if (log.isDebugEnabled()) {
                log.debug("Checking if we can deal a new hand...");
            }
            FsmProcessObject pgpo = (FsmProcessObject) po;

            result = workflowProcessorService.isStartable(pgpo.getId());

            if (log.isDebugEnabled()) {
                log.debug("Checking if we can deal a new hand: " + result);
            }
        }
