...
@Component
public class IsPostBigBlind extends AbstractCondition {
    /**
     * Field description
     */
    private final static Logger log = LoggerFactory.getLogger(IsPostBigBlind.class);
     ...
     */
    @Autowired
    public IsPostBigBlind(WorkflowProcessorService workflowProcessorService) {
    ...
    public boolean evaluate(ProcessObject po) {
        ...
        if (po instanceof FsmProcessObject) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Checking if it's time to post big blind for hand ID: " + pgpo.getHandId());
            }

            result = workflowProcessorService.isBigBlindPostable(pgpo.getHandId());

            if (log.isDebugEnabled()) {
                log.debug("Checking if it's time to post big blind for hand ID: " + pgpo.getHandId() + " : " + result);
            }
        }
