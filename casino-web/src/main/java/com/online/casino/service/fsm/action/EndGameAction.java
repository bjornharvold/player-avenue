...
public class EndGameAction extends AbstractAction {
     ...
     */
    @Autowired
    public EndGameAction(WorkflowProcessorService workflowProcessorService) {
    ...
    public void execute(ProcessObject po) {
        if (po instanceof FsmProcessObject) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Executing end game on hand id: " + pgpo.getHandId());
            }

            workflowProcessorService.doEndGame(pgpo.getId(), pgpo.getHandId());

            if (log.isDebugEnabled()) {
                log.debug("Executing end game on hand id: " + pgpo.getHandId() + " complete");
            }
        }
    }
}
