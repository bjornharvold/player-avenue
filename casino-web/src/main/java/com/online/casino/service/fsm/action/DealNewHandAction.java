 ...
 */
@Component
public class DealNewHandAction extends AbstractAction {

    /**
     * Field description
     */
    private final static Logger log = LoggerFactory.getLogger(DealNewHandAction.class);
     ...
     */
    @Autowired
    public DealNewHandAction(WorkflowProcessorService workflowProcessorService) {
    ...
    public void execute(ProcessObject po) {
        if (po instanceof FsmProcessObject) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Dealing new hand for pokergame id: " + pgpo.getId());
            }

            Hand hand = workflowProcessorService.doDealNewHand(pgpo.getId());

            pgpo.setHandId(hand.getId());

            if (log.isDebugEnabled()) {
                log.debug("Dealt new hand with id: " + pgpo.getHandId());
            }
        }
    }
}
