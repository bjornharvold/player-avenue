...
public class FoldAction extends AbstractAction {
     ...
     * Field description
     */
    private final static Logger log = LoggerFactory.getLogger(FoldAction.class);
     ...
     */
    @Autowired
    public FoldAction(WorkflowProcessorService workflowProcessorService) {
    ...
    public void execute(ProcessObject po, Object triggerEvent) {
        if (po instanceof FsmProcessObject && triggerEvent instanceof FoldEvent) {
            FsmProcessObject pgpo = (FsmProcessObject) po;
            FoldEvent event = (FoldEvent) triggerEvent;

            if (log.isDebugEnabled()) {
                log.debug("Gambler with id: " + event.getGamblerId() + " folds");
            }

            workflowProcessorService.doFold(event);

            if (log.isDebugEnabled()) {
                log.debug("Gambler with id: " + event.getGamblerId() + " folded successfully");
            }
        }
    }
}
