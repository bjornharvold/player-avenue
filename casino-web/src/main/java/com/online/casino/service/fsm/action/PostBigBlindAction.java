 ...
 */
@Component
public class PostBigBlindAction extends AbstractAction {
     ...
     */
    @Autowired
    public PostBigBlindAction(WorkflowProcessorService workflowProcessorService) {
    ...
    public void execute(ProcessObject po) {
        if (po instanceof FsmProcessObject) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Posting big blind for hand ID: " + pgpo.getHandId());
            }

            workflowProcessorService.doPostBigBlind(pgpo.getId(), pgpo.getHandId());

            if (log.isDebugEnabled()) {
                log.debug("Posted big blind for hand ID: " + pgpo.getHandId());
            }
        }
    }
}
